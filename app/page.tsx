...
export default function Home() {
  const { state, saveState, update, retry, recoveryKey, restoreRecoveryKey } = useDecision();
  ...
  const visible = showAll || state?.orientation === "explore"
    ? parties
  ...
  function togglePartyVote(slug: string, vote: "positive" | "out") {
    update((previous) => {
      ...
      if (partyStatus[slug] === vote) delete partyStatus[slug];
      else partyStatus[slug] = vote;
    ...
    <main className="shell">
      ...
      {!state ? (
        ...
        <section className="panel orientation-panel" aria-labelledby="orientation-title">
          ...
          <p className="muted small" style={{ padding: "0 24px 22px", margin: 0 }}>ללא הרשמה או כניסה. הבחירות שלך יישמרו בדפדפן הזה ובמסד נתונים נפרד באמצעות מפתח פרטי למכשיר.</p>
        </section>
      ) : <>
      ...
      <div className="workspace-grid">
        <section className="panel" aria-labelledby="priorities-title">
          <div className="panel-header">
            ...
            <SaveIndicator status={saveState} retry={retry} />
          </div>
            <div className="panel-body">
              ...
              <div className="tags" aria-label="נושאים חשובים">
                {[...priorityOptions, ...state.customTags].map((label) => (
              ...
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 10, marginTop: 10 }}>
                <span className="muted small">הטקסט נשמר בלחיצה.</span>
                <button className="button" type="button" disabled={generalDraft === null || generalDraft === state.generalNotes}
                  onClick={() => { const value = generalDraft ?? state.generalNotes; update((previous) => ({ ...previous, generalNotes: value }), "general_note"); setGeneralDraft(null); }}>
                  שמור מחשבה
                </button>
              </div>
              <hr className="divider" />
              <details className="recovery-details">
                ...
                <p className="muted small">אין חשבון. המפתח הזה נותן גישה לכל ההחלטות שלך. שמור אותו במקום פרטי אם תרצה להמשיך במכשיר אחר. שחזור מפתח אחר יחליף את המפה המוצגת בדפדפן הזה.</p>
        ...
        <div className="section-stack">
          <section className="panel" aria-labelledby="parties-title">
            <div className="panel-header">
              <h2 id="parties-title">הרשימות לבדיקה</h2>
              <span className="muted small">{visible.length} רשימות</span>
            </div>
            <div className="party-list">
              {sortedVisible.map((party, index) => (
                ...
                <div className="party-card" data-reaction={rank(party.slug) === 0 ? "like" : rank(party.slug) === 2 ? "dislike" : undefined} style={{ "--party-accent": party.color } as React.CSSProperties}>
                  ...
                  <a className="party-card-link" href={`/party/${party.slug}`}>
